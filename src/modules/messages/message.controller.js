...
import pusher from "../../utils/Pusher/PusherConfig.js";
...
export const sendMesg = async (req, res, next) => {
  try {
    ...
    const newMessage = await messageModel.create({ contant: messageContent, userId: receiverId, });

    await notificationModel.create({ recipient: receiverId, content: "You got a new Message ! 🤫", });
    const totalMessagesCount = await messageModel.countDocuments({ userId: receiverId, });




   const test = await pusher.trigger(`user-${receiverId}`, `receiveRealTimeMessage`, {
